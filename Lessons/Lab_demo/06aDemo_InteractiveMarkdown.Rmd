...
my_plot <- nutrient_data %>% 
  ...
  scale_x_date(
    date_breaks = "1 years",
    date_labels = '%Y') +
#17 is a triangle shape, 15 is square, look up # corresponding to shape online  
...
#input function parameters and call function to view plot
...
selectInput(
  ...
  choices = c('tn_ug', 'tp_ug'),
#need to provide a default choice  
  selected = 'tn_ug'
)
...
sliderInput(
  ...
  #provides interval for slider
  step = 1,
  round = 1,
  #adds or removes comma separator from numeric values
  sep = ''
)

# Create interactive renderPlot object to display the plot that changes based on slider input
renderPlot(
  # created with the plot_it function
  plot_it(
    input$the_nutrient, 
    #need two slider inputs because two ends of slider
    input$year_slider[1],
    input$year_slider[2])
)

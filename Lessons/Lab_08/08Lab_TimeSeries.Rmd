...
#Create a sequence that filters through the first and last dates of the dataframe, specify the time sequence for the date

Months <- as.data.frame(seq.Date(from=as.Date("1906/01/01"), 
                                to=as.Date("2016/12/01"), 
                                by="month"))
colnames(Months) <- "Date"
#join replaces any missing rows with NA, number of observations would decrease
...
#Use a function to isolate the first values for month and date
f_month <- month(first(BR_Complete$Date))
f_year <- year(first(BR_Complete$Date))

#use first functions as start for time series
Temp.ts <- ts(BR_Complete$Temperature_C, start = c(f_year, f_month), frequency = 12)
Rain.ts <- ts(BR_Complete$Rainfall, start = c(f_year, f_month), frequency = 12)
...
Temp.decomp <- stl(Temp.ts, s.window = "periodic")
plot(Temp.decomp)

Rain.decomp <- stl(Rain.ts, s.window = "periodic")
plot(Rain.decomp)


# Exercise 9: Use the times series object generated with the stl() to extract the 
# components of each series and turn them into data frames. Use 3 columns for the 3 components from the ts
Temp.extract <- as.data.frame(Temp.decomp$time.series[,1:3])

#Add observed and date columns
Temp.extract <- mutate(Temp.extract,
